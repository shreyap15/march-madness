...
FIELD_2026 = {
    ...
    "Midwest": {
        1: "Michigan", 16: "Howard", 8: "Georgia", 9: "Saint Louis",
    ...
    "South": {
        1: "Florida", 16: ["Prairie View A&M", "Lehigh"], 8: "Clemson", 9: "Iowa",
    ...
    "West": {
        1: "Arizona", 16: "Long Island (LIU)", 8: "Villanova", 9: "Utah State",
...
REGION_TO_LETTER = {"East": "W", "Midwest": "X", "South": "Y", "West": "Z"}
...
def _build_name_map(teams: pd.DataFrame) -> Dict[str, int]:
    ...
    for raw, alias in NAME_ALIASES.items():
        if _norm(alias) in name_to_id:
            name_to_id[_norm(raw)] = name_to_id[_norm(alias)]
    return name_to_id


def _resolve_team_id(name: str, name_to_id: Dict[str, int]) -> int | None:
    return name_to_id.get(_norm(name))
...
def build_bracket_2026(
    out_path="submissions/men/2026/bracket_2026.csv",
    preds_path="submissions/men/2026/MNCAATourneyPredictions_with_preds.csv",
    ...
    teams_df = raw["teams"][["TeamID", "TeamName"]].copy()
    name_to_id = _build_name_map(teams_df)
    ...
    if use_manual_field:
        ...
        for region, seeds_in_region in FIELD_2026.items():
            ...
            for seed_num, team_name in seeds_in_region.items():
                ...
                if team_id is None:
                    raise ValueError(f"Could not resolve {team_name}")

                seed_to_team[f"{letter}{seed_num:02d}"] = team_id
    else:
    ...
    for _, row in slots.iterrows():
        strong_id = seed_to_team.get(row["StrongSeed"], np.nan)
        weak_id = seed_to_team.get(row["WeakSeed"], np.nan)

        rows.append({
            "Season": season,
            "Slot": row["Slot"],
            "Round": float(row["Slot"][1]) if str(row["Slot"]).startswith("R") else np.nan,
            "StrongSeed": row["StrongSeed"],
            "StrongTeamID": strong_id,
            "StrongTeamName": teams_df.at[strong_id, "TeamName"] if strong_id in teams_df.index else np.nan,
            "WeakSeed": row["WeakSeed"],
            "WeakTeamID": weak_id,
            "WeakTeamName": teams_df.at[weak_id, "TeamName"] if weak_id in teams_df.index else np.nan,
        })
    ...
    if preds is not None:
        ...
        for _, r in preds.iterrows():
            try:
                _, t1, t2 = r["ID"].split("_")
                t1, t2 = int(t1), int(t2)
                p = float(r["Pred"])
            except Exception:
                continue

            w, l = (t1, t2) if p >= 0.5 else (t2, t1)
            wl_rows.append({"WTeamID": w, "LTeamID": l})

        wl_df = pd.DataFrame(wl_rows)
        wl_df.to_csv(out_path.replace("bracket_2026.csv", "NCAATourneyPredictions.csv"), index=False)
